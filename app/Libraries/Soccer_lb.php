...
//        http://data.nowgoal.com/analysis/1496576.html
        ...
        file_put_contents('/home/spoto2/html/writable/uploads/analysis/' . $seq . '.json', json_encode($analysis_data));
...
//            var_dump($arr);
    ...
    private function roundScore($res, $teamSelect, $division, $thisRound)
    {
        ...
        for ($i = 1; $i < $thisRound; $i++) {
            ...
            foreach ($arr as $row) {
                ...
                if ($row[2] !== '推迟|推遲|Delay') {//推迟|推遲|Delay 예외처리
    ...
    public function tech_func($arr, $_techPlayer_arr, $type)
    {
        ...
        foreach ($arr[$type]['value'] as $row) {
            $_arr = array();
            if ($row[0] !== 133853 && $row[0] !== 183239 && $row[0] !== 150975) { // 133853 선수정보 누락으로 오류 예외처리
                for ($i = 0; $i < count($row); $i++) {
...
//        $file = WRITEPATH.'/uploads/2020-01-15.json';
//
//        $lastmod = date("Y.m.d H:i:s", filemtime(($file)));
//
//        echo("최종 수정 시간은 $lastmod 입니다.");
