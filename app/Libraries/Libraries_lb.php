...
class Libraries_lb extends BaseController
{
        ...
        if (!empty($files)) {
            ...
            foreach ($files as $key) {
                if ($key->getName()) {
                    ...
                    $key->move(ROOTPATH . 'public/uploads/'. $folderName .'/'. $mdFolder .'/', $randName);
    ...
    {
        $path = ROOTPATH . 'public/uploads/' . $folderName . '/' . $mdFolder ;
        if (!is_dir($path)) {
            mkdir($path);
        }
    }
    ...
    {
        if (is_dir(ROOTPATH . 'public/uploads/'.$folderName .'/'. $mdFolder['files_folder']) && !empty($mdFolder['files_folder'])) {
            delete_files(ROOTPATH . 'public/uploads/' .$folderName. '/' . $mdFolder['files_folder'], true);
            rmdir(ROOTPATH . 'public/uploads/'.$folderName.'/' . $mdFolder['files_folder']);
        }
    }
    //세션,파일 제거
    public function sessionRemove()
    {
        ...
        if (!empty($files['folder_name'])) {
            delete_files(ROOTPATH . 'public/uploads/board/'. $files['folder_name'], true);
            if (is_dir(ROOTPATH . 'public/uploads/board/'. $files['folder_name'])) {
                rmdir(ROOTPATH . 'public/uploads/board/'. $files['folder_name']);
            }
        }
    }
}
