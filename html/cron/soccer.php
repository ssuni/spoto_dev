...
class Soccer
{
    ...
    public function mainData($res)
    {
        ...
        return array('A' => $_tmp, 'B' => $_tmpB, 'today' => $_today, 'tomorrow' => $_tomorrow, 'result' => $_result, 'flag' => flag);
    }
    ...
    public function resultData($date, $res)
    {
        ...
        $_idx = array('game_idx', 'league_key', 'home_idx', 'away_idx', 'home_name', 'away_name', 'year', 'game_progress',
            ...
            'home_yellow_card', 'away_yellow_card', '16', '17', '18', 'append_content', '20', '21', '22', 'home_c', 'away_c','division_day');
        ...
        foreach ($A[2] as $row) {
            ...
            for ($i = 0; $i < count($arr); $i++) {
                ...
                if ($i == 20) {
                    for ($c = 0; $c < count($_continent); $c++) {
                        if ($arr[$i] == $_continent[$c][0]) {
                            ...
                            $arr_idx['continent_idx'] = $_continent[$c][0];
                        }
                    }
                }
                ...
                if ($i == 6) {
                    ...
                    $_division_day = date("Y-m-d", strtotime($_time . '+9 hour'));
                    $arr_idx[$_idx[$i]] = $_time;
                    $arr_idx['division_day'] = $_division_day;
                } else {
    ...
    public function opponentData($res)
    {
        ...
        return array('opponent' => $_result);
    }

    public function _createDateRange($startDate, $endDate, $format = "Y-m-d")
    {
    ...
    public function calendar()
    {
        ...
        $end_date = date("Y-m-d", strtotime($date . '8 day'));
        $date = $this->_createDateRange($prev_date, $end_date);
...
//            $url = 'http://data.nowgoal.com/MatchByCountry.aspx?date='.$date.'&orderby=time&type=2';
        ...
        $arr_append = array('cal_count' => $count, 'cal_result' => $_calendar);
        file_put_contents('/home/spoto/html/writable/uploads/calendar/' . date('Y-m-d') . '.json', json_encode($arr_append));
...
$tommorow = date("Y-m-d", strtotime("+1 day")); //데이터
$_arrUrl = array(
...
//    'tommorow' => 'http://www.nowgoal.com/GetScheduleWithTimeZone.aspx?date=' . $tommorow . '&order=time&timezone=9'
    'main' => 'http://www.nowgoal.group/gf/data/bf_kr.js?' . time(),
    'detail' => 'http://www.nowgoal.group/gf/data/detail.js?' . time(),
    'opponent' => 'http://www.nowgoal.group/gf/data/panlu_en.js?' . time(),
    'result' => 'http://www.nowgoal.group/GetScheduleWithTimeZone.aspx?date=' . $date . '&order=time&timezone=9',
    'tommorow' => 'http://www.nowgoal.group/GetScheduleWithTimeZone.aspx?date=' . $tommorow . '&order=time&timezone=9'
);
...
//$data = array_merge($mainData, $detailData, $_odd, $opponentData, $calendar);
$data = array_merge($mainData, $detailData, $opponentData, $calendar);
//$data = array_merge($mainData,$detailData),,$opponentData;
