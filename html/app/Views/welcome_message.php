...
<script src="https://spoto.com/js/socket.io.js"></script>
<div class="wrap">
    ...
    <!--        </div>-->
    <!--        <div>-->
    <!--            <video autoplay="" loop="" muted="" style="width:100%;"><source src="-->
    <?php //echo base_url('/img/test.mp4')?><!--" type="video/mp4"></video>-->
    <!--        </div>-->
    <!--    </div>-->
            ...
            foreach ($today as $to) {
                ...
                <tr>
                    <?php if ($to['league_key'] == $B[$to['league_key'] - 1]['league_key']) { ?>
                        <td style="background-color: <?php echo $B[$to['league_key'] - 1]['color']; ?>">
                        <span style="color:white"><?php echo $B[$to['league_key'] - 1]['league_short']; ?>
                        </span>
                        </td>
                    <?php } ?>
                    <td>
...
<div id="soundsHome"></div>
<!--<audio autoplay controls id="bee-sound" src="/audio/Golf.mp3" style="display:none;"></audio>-->

</body>


<script>
    var source = "/audio/Golf.mp3"
    ...
    $(function () {
        var socket = io.connect("wss://io.spoto.com/bridgeTest", {
        ...
        socket.on('toclient', function (data) {
            ...
            console.log(data);
        }).on('soccerEvent', function (data) {
            body.dispatchEvent(new Event('click'));
            console.log(data);
        }).on('ftEvent', function (data) {
            console.log(data);
        });

        socket.emit('fromclient', {msg: 'fromclient'});
        socket.emit('connect', {test: 'test'});
    })
</script>
<script>
    function team(idx) {
