                        ...
                        chart.data.datasets.forEach(function(dataset, i) {
                            ...
                            if (!meta.hidden) {
                                meta.data.forEach(function(element, index) {
                                    ...
                                    // 차트 위 값 표시
                                    if(ctx.canvas.id === 'h_keeper_e_shoot' || ctx.canvas.id === 'a_keeper_e_shoot' || ctx.canvas.id === 'h_keeper_defense' || ctx.canvas.id === 'a_keeper_defense' || ctx.canvas.id === 'h_defense_rate' || ctx.canvas.id === 'a_defense_rate') {
                                        var padding = 1;
                                        var position = element.tooltipPosition();
                                        var first_data = chart.data.datasets[0].data;
...
console.log(uni_total_idx);
            ...
            detail_graph_draw: function() {
                ...
                var id_arr = ['goal_15_time','loss_15_time','goal_10_time','loss_10_time'],
                    first_data_arr = [goal_15_h_data,loss_15_h_data,goal_10_h_data,loss_10_h_data],
                    second_data_arr = [goal_15_a_data,loss_15_a_data,goal_10_a_data,loss_10_a_data];
                for(var i=0; i<4; i++) {
                    // var h_data_15 = [first_data_arr[i].time_0_15, first_data_arr[i].time_16_30, first_data_arr[i].time_31_45,
                    //         first_data_arr[i].time_46_60, first_data_arr[i].time_61_75, first_data_arr[i].time_76_90],
                    //     a_data_15 = [second_data_arr[i].time_0_15, second_data_arr[i].time_16_30, second_data_arr[i].time_31_45,
            ...
            detail_line_graph: function(arr) {
                ...
                //var detail_chart = new Chart(ctx, config);
                // Chart.plugins.register({
                //     afterDatasetsDraw: function(chart) {
                //         var ctx = chart.ctx;
                //         chart.data.datasets.forEach(function(dataset, i) {
    ...
    _Analysis_event = (function () {
        return {
            ...
            game_result_stats_filter: function(json_data,a_h_t_div,div_arr,content_adr,content_class_name,content_index,type) {
...
//모든 리그 체크박스 클릭 이벤트
    ...
    $('#analysis_popup').on('click', '#tap_menu div', function() {
        ...
        if(_this.hasClass('home_game') === true){
            a_h_t_div = 'home';
            _Analysis_event.game_result_stats_filter(json_data,a_h_t_div,div_arr,content_adr,content_class_name,content_index,type);
        }else if(_this.hasClass('away_game') === true){
            a_h_t_div = 'away';
            _Analysis_event.game_result_stats_filter(json_data,a_h_t_div,div_arr,content_adr,content_class_name,content_index,type);
        }else if(_this.hasClass('all_game') === true){
