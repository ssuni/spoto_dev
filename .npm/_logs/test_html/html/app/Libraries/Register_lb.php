...
class Register_lb extends BaseController
{
    ...
    public function usersValid()
    {
        return [
            ...
            'age' => 'required|max_length[3]|is_natural',
            'gender' => 'required',
        ];
    }
    //로그인 유효성을 검사합니다
    ...
    {
        $data['errors'] = $this->signupValidConf($data);

        if (empty($data['errors'])) {
            $data['pass'] = $this->encrypt($data['pass']);
            $data['signup_ip'] = $this->request->getIPAddress();
            $this->Register_m->signupInsert($data);
        } else {
            return $data['errors'];
        }
    }
    ...
    public function loginConf($data)
    {
        ...
        if (empty($data['errors'])) {
            $result = $this->Register_m->selectRow($data['id'], 'id');
    ...
    public function snsCheck($code, $snsName)
    {
        ...
        $resultUser = $this->Register_m->selectRow($id, 'id');

        if (empty($resultUser)) {
            $data['author']['sns_id'] = $result->id;
            $data['author']['id'] = $id;
            $data['author']['pass'] = $result->id;
            $data['author']['sns_type'] = $this->snsType($snsName);
            return $data;
        } else {
    ...
    {
        $result = $this->Register_m->selectRow($idx);
        $result['pass'] = $this->decrypt($result['pass']);
    ...
    {
        $result = $this->Register_m->selectRow($data['idx']);
        ...
        if (empty($result['errors'])) {
            ...
            $this->Register_m->updateRow($data);
        } else {
    ...
    private function kakaoUrl()
    {
        ...
        $redirectUrl = '&redirect_uri=https://spoto.com/Register/kakaoResponse';
    ...
    private function naverUrl()
    {
        ...
        $redirectUrl = '&redirect_uri=https://spoto.com/Register/naverResponse';
        $response = '&response_type=code';
        $state = '&state=' . $state;
    ...
    private function googleUrl()
    {
        ...
        $redirectUrl = '&redirect_uri=https://spoto.com/Register/googleResponse';
        $response = '&response_type=code';
    ...
    //카카오로그인 성공시 회원고유 id값을 받아옵니다.
    public function kaLogin($code)
    {
        $app_key = '5093f115f5ddad453b971548da8f09f8';
        $redirect_uri = 'https://spoto.com/Register/kakaoResponse';
    ...
    public function naLogin($state)
    {
        if (!empty($state)) {
            ...
            $redirectUri = '&redirect_uri=https://spoto.com/Register/naverResponse';
    ...
    public function glLogin($code)
    {
        ...
        $client->setRedirectUri('https://spoto.com/Register/googleResponse');
